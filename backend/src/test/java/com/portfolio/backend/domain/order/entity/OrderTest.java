...
class OrderTest {
    ...
    void createOrder_Success() {
        ...
        assertEquals(OrderStatus.PENDING_STOCK_REDUCTION, order.getOrderStatus());
    ...
    @DisplayName("결제 대기 상태가 아닐 때 결제 완료로 변경하면 예외가 발생한다")
    @Test
    void paymentCompleted_InvalidState_ThrowsException() {
        // given
        User user = UserTestFixtures.createUser();
        ...
        Order order = new Order(user, totalPrice, orderItems, deliveryInfo);
        // 재고 확인 완료 상태로 변경하지 않고 바로 결제 완료 시도
    ...
    @DisplayName("주문 완료 상태가 아닐 때 취소하면 예외가 발생한다")
    @Test
    ...
    @Test
    void failed_InvalidState_ThrowsException() {
        // given
        User user = UserTestFixtures.createUser();
